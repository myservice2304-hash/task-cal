...
export default function App() {
  ...
  const [memos, setMemos] = useState(() => {
    try { return JSON.parse(localStorage.getItem("taskcal_memos")||"{}"); } catch { return {}; }
  });
  ...
  function saveMemo() {
    ...
    const clientMemos = memos[memoClient] || [];
    if (editMemo.id) {
      setMemos(p => ({ ...p, [memoClient]: clientMemos.map(m => m.id===editMemo.id ? editMemo : m) }));
    } else {
      const newMemo = { ...editMemo, id: Date.now(), createdAt: new Date().toLocaleDateString("ja-JP") };
      setMemos(p => ({ ...p, [memoClient]: [newMemo, ...clientMemos] }));
    }
    setShowMemoForm(false);
  }
  function deleteMemo(client, id) {
                    ...
                    <div key={client} style={{ marginBottom:8 }}>
                      ...
                      <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between",
                        ...
                        <div style={{ display:"flex", alignItems:"center", gap:8 }}>
                          <button onClick={e=>{ e.stopPropagation(); setMemoClient(client); setShowMemo(true); }}
                            style={{ background:cc.bg, border:`1px solid ${cc.accent}`, color:cc.accent,
                              borderRadius:16, padding:"4px 10px", fontSize:12, fontWeight:700, cursor:"pointer",
                              display:"flex", alignItems:"center", gap:4 }}>
                            📝 メモ{(memos[client]||[]).length>0?` (${(memos[client]||[]).length})`:""}
                          </button>
                          <span onClick={()=>setCollapsedClients(p=>({...p,[client]:!p[client]}))}
                            style={{ fontSize:18, color:"#bdc3c7", cursor:"pointer",
                              transform:isCollapsed?"rotate(-90deg)":"rotate(0deg)", transition:"transform 0.2s" }}>▾</span>
                        </div>
                      </div>
                      {/* タスク一覧 */}
                      {!isCollapsed && (
  ...
  return (
    <div style={{ height:"100vh", display:"flex", flexDirection:"column",
      ...
      {/* Memo List */}
      <BottomSheet open={showMemo&&!showMemoForm} onClose={()=>setShowMemo(false)}
        title={memoClient ? `📝 ${memoClient}` : "メモ"}>
        {memoClient && (()=>{
          const cc = clientColorMap[memoClient]||CLIENT_COLORS[0];
          const clientMemos = memos[memoClient]||[];
          return (
            <div style={{ padding:"16px 20px 40px" }}>
              <button onClick={()=>{ setEditMemo({id:null,title:"",body:""}); setShowMemoForm(true); }}
                style={{ width:"100%", background:"#1a2740", color:"#fff", border:"none",
                  padding:"13px", borderRadius:12, fontWeight:800, fontSize:15, cursor:"pointer", marginBottom:16 }}>
                ＋ 新しいメモを追加
              </button>
              {clientMemos.length===0?(
                <div style={{ textAlign:"center", padding:"40px 0", color:"#bdc3c7" }}>
                  <div style={{ fontSize:40, marginBottom:8 }}>📝</div>
                  <div style={{ fontSize:14 }}>メモがありません</div>
                  <div style={{ fontSize:12, marginTop:4 }}>上のボタンから追加できます</div>
                </div>
              ):(
                <div style={{ display:"flex", flexDirection:"column", gap:10 }}>
                  {clientMemos.map(memo=>(
                    <div key={memo.id} style={{ background:"#fff", borderRadius:12,
                      border:`2px solid ${cc.accent}20`, boxShadow:"0 1px 6px rgba(0,0,0,0.06)" }}>
                      <div style={{ padding:"12px 14px 8px", borderBottom:"1px solid #f0f0f0",
                        display:"flex", alignItems:"center", justifyContent:"space-between" }}>
                        <div>
                          <div style={{ fontSize:15, fontWeight:800, color:"#1a2740" }}>{memo.title||"（タイトルなし）"}</div>
                          <div style={{ fontSize:11, color:"#95a5a6", marginTop:2 }}>{memo.createdAt}</div>
                        </div>
                        <div style={{ display:"flex", gap:6 }}>
                          <button onClick={()=>{ setEditMemo({...memo}); setShowMemoForm(true); }}
                            style={{ background:"#f0f7ff", border:"none", color:"#2980b9",
                              borderRadius:8, padding:"5px 10px", fontSize:12, fontWeight:700, cursor:"pointer" }}>編集</button>
                          <button onClick={()=>deleteMemo(memoClient, memo.id)}
                            style={{ background:"#fdecea", border:"none", color:"#e74c3c",
                              borderRadius:8, padding:"5px 10px", fontSize:12, fontWeight:700, cursor:"pointer" }}>削除</button>
                        </div>
                      </div>
                      {memo.body&&(
                        <div style={{ padding:"10px 14px", fontSize:14, color:"#2c3e50",
                          whiteSpace:"pre-wrap", lineHeight:1.6 }}>{memo.body}</div>
                      )}
                    </div>
                  ))}
                </div>
      ...
      {/* Memo Form */}
      <BottomSheet open={showMemo&&showMemoForm} onClose={()=>setShowMemoForm(false)}
        title={editMemo?.id?"メモを編集":"新しいメモ"}>
        {editMemo&&(
          <div style={{ padding:"16px 20px 40px" }}>
            <div style={{ marginBottom:16 }}>
              <label style={{ fontSize:12, fontWeight:700, color:"#7f8c8d", display:"block", marginBottom:6 }}>タイトル</label>
              <input value={editMemo.title} onChange={e=>setEditMemo({...editMemo,title:e.target.value})}
                placeholder="例：初回MTG議事録"
                style={{ width:"100%", padding:"13px 14px", border:"2px solid #e8ecf0", borderRadius:10,
                  fontSize:16, outline:"none", boxSizing:"border-box", background:"#f8fafc" }} />
            </div>
            <div style={{ marginBottom:20 }}>
              <label style={{ fontSize:12, fontWeight:700, color:"#7f8c8d", display:"block", marginBottom:6 }}>内容</label>
              <textarea value={editMemo.body} onChange={e=>setEditMemo({...editMemo,body:e.target.value})}
            ...
            <div style={{ display:"flex", gap:10 }}>
              <button onClick={saveMemo}
                style={{ flex:2, background:"#1a2740", color:"#fff", border:"none",
                  padding:"15px", borderRadius:12, fontWeight:800, fontSize:15, cursor:"pointer" }}>保存する</button>
              <button onClick={()=>setShowMemoForm(false)}
                style={{ flex:1, background:"#f0f4f8", color:"#7f8c8d", border:"none",
                  padding:"15px", borderRadius:12, fontWeight:700, fontSize:15, cursor:"pointer" }}>戻る</button>
            </div>
          </div>
        )}
      </BottomSheet>
      ...
      {/* メモ一覧 */}
      <BottomSheet open={showMemo} onClose={()=>setShowMemo(false)} title={`📝 ${memoClient} のメモ`}>
        {memoClient && (()=>{
          const cc = clientColorMap[memoClient]||CLIENT_COLORS[0];
          const clientMemos = memos[memoClient]||[];
          return (
            <div style={{ padding:"12px 16px 40px" }}>
              <button onClick={()=>{ setEditMemo({id:null,title:"",body:""}); setShowMemoForm(true); }}
                style={{ width:"100%", background:cc.accent, color:"#fff", border:"none",
                  padding:"13px", borderRadius:12, fontWeight:800, fontSize:15, cursor:"pointer", marginBottom:16 }}>
                ＋ 新しいメモを追加
              </button>
              {clientMemos.length===0?(
                ...
                <div style={{ display:"flex", flexDirection:"column", gap:10 }}>
                  {clientMemos.map(memo=>(
                    <div key={memo.id} style={{ background:"#fff", borderRadius:12, padding:"14px 16px",
                      boxShadow:"0 1px 6px rgba(0,0,0,0.07)", borderLeft:`4px solid ${cc.accent}` }}>
                      <div style={{ display:"flex", alignItems:"flex-start", justifyContent:"space-between", marginBottom:6 }}>
                        <div style={{ flex:1, minWidth:0 }}>
                          <div style={{ fontSize:15, fontWeight:800, color:"#1a2740", marginBottom:2 }}>{memo.title||"（タイトルなし）"}</div>
                          <div style={{ fontSize:11, color:"#bdc3c7" }}>{memo.createdAt}</div>
                        </div>
                        <div style={{ display:"flex", gap:6, flexShrink:0, marginLeft:8 }}>
                          <button onClick={()=>{ setEditMemo({...memo}); setShowMemoForm(true); }}
                            style={{ background:"#f0f4f8", border:"none", borderRadius:8, padding:"5px 10px",
                              fontSize:12, fontWeight:700, color:"#2c3e50", cursor:"pointer" }}>編集</button>
                          <button onClick={()=>deleteMemo(memoClient, memo.id)}
                            style={{ background:"#fdecea", border:"none", borderRadius:8, padding:"5px 10px",
                              fontSize:12, fontWeight:700, color:"#e74c3c", cursor:"pointer" }}>削除</button>
                        </div>
                      </div>
                      {memo.body&&(
                        <div style={{ fontSize:14, color:"#2c3e50", lineHeight:1.7, whiteSpace:"pre-wrap",
      ...
      {/* メモ編集フォーム */}
      <BottomSheet open={showMemoForm} onClose={()=>setShowMemoForm(false)}
        title={editMemo?.id?"メモを編集":"新しいメモ"}>
        {editMemo&&(
          <div style={{ padding:"16px 20px 40px" }}>
            <div style={{ marginBottom:16 }}>
            ...
            <button onClick={saveMemo}
              style={{ width:"100%", background:"#1a2740", color:"#fff", border:"none",
                padding:"16px", borderRadius:12, fontWeight:800, fontSize:16, cursor:"pointer" }}>
              保存する
            </button>
          </div>
        )}
